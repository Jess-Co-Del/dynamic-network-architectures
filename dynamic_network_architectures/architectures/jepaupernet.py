...
class FPNNeck(nn.Module):
    ...
    Takes N feature maps of identical resolution (14×14 from ViT)
    and produces N output maps at progressively doubled resolutions:
...
class UPerHead(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.num_fpn_levels = len(pool_scales)

        # Lateral 1×1 convs: channel reduction for each tapped ViT layer
        self.lateral_convs = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(in_channels, fpn_channels, kernel_size=1, bias=False),
                nn.GroupNorm(8, fpn_channels),
            )
            for _ in range(self.num_fpn_levels)   # one per tapped layer
        ])
        ...
        # PPM output: original + 4 pooled → (1 + len(pool_scales)) × fpn_channels
        ppm_out_channels = fpn_channels + len(pool_scales) * fpn_channels

        self.ppm_bottleneck = nn.Sequential(
            nn.Conv2d(ppm_out_channels, fpn_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(fpn_channels),
            nn.ReLU(inplace=True),
        )
        ...
        self.fpn_bottleneck = nn.Sequential(
            nn.Conv2d(self.num_fpn_levels * fpn_channels, out_channels,
    ...
    def forward(self, features: List[Tensor]) -> Tensor:
        ...
        # Use index -1 for PPM (most shallow)
        semantic = laterals[-1]   # [B, 256, 14, 14]
        ppm_outs = [semantic]
        ...
        # Add the PPM-enriched features
        fpn_list = list(laterals[:-1]) + [ppm_fused]
        
        fpn_outs = self.fpn(fpn_list)

        # ── Upsample all levels to the finest resolution (112×112) ────────
        target_h, target_w = fpn_outs[-1].shape[-2:]  # 112×112
        aligned = [
        ...
        return fused   # [B, out_channels, 112, 112]
...
class IJEPAUPerNetSeg(nn.Module):
    ...
    def __init__(
        ...
        self.head = UPerHead(in_channels=hidden_size, out_channels=num_classes)
    ...
    def forward(self, pixel_values: Tensor) -> Tensor:
        ...
        # hidden_states: tuple of (num_layers) tensors, each [B, 196, 1408]
        ...
        # logits_low = self.fpn(fpn_features)

        # fpn_features: [B,256,14], [B,256,28], [B,256,56], [B,256,112]
        # ── 3. UPerNet head: PPM + FPN + concat ──────────────────────
        fpn_features = self.head(multi_scale)    # [B, num_classes, 112, 112]
