...
from dynamic_network_architectures.building_blocks.simple_conv_blocks import ConvDropoutNormReLU, CondConvDropoutNormReLU
from dynamic_network_architectures.building_blocks.regularization import DropPath, SqueezeExcite
import numpy as np


class CondBasicBlockD(nn.Module):
    def __init__(self,
        ...
        self.conv1 = CondConvDropoutNormReLU(conv_op, input_channels, output_channels, kernel_size, stride, conv_bias,
        ...
        if time_embedding_dim is not None:
            self.skip_pool = lambda x: x
            self.skip_conv = lambda x, y : x
            if has_stride:
                self.skip_pool = get_matching_pool_op(conv_op=conv_op, adaptive=False, pool_type='avg')(stride, stride)
            if requires_projection:
                self.skip_conv = CondConvDropoutNormReLU(conv_op, input_channels, output_channels, 1, 1, False, norm_op,
                                        norm_op_kwargs, None, None, None, None, False, time_embedding_dim
                                    )
        elif time_embedding_dim is None:
            ops = []
            if has_stride:
    ...
    def forward(self, x: torch.Tensor, tembed: torch.Tensor = None):
        if tembed is not None:
            #print("applying temb -->",self.stride, self.requires_projection, tembed.shape, x.shape)
            residual = self.skip_pool(x)
            residual = self.skip_conv(residual, tembed)
            out = self.conv1(x, tembed)
            out = self.conv2(out, tembed)
            if self.apply_stochastic_depth:
...
class StackedResidualBlocks(nn.Module):
    def __init__(self,
                 ...
                 nonlin: Union[None, Type[torch.nn.Module]] = None,
                 nonlin_kwargs: dict = None,
                 block: Union[Type[BasicBlockD], Type[BottleneckD], Type[CondBasicBlockD]] = BasicBlockD,
        ...
        assert block in [BasicBlockD, BottleneckD, CondBasicBlockD], 'block must be BasicBlockD or BottleneckD'
        ...
        if block == BasicBlockD:
            ...
            blocks = nn.ModuleList([
                ...
                *[block(conv_op, output_channels[n - 1], output_channels[n], kernel_size, 1, conv_bias, norm_op,
                        norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, stochastic_depth_p,
                        squeeze_excitation, squeeze_excitation_reduction_ratio, 
                        time_embedding_dim) for n in range(1, n_blocks)]]
            )
        else:
            blocks = nn.Sequential(
