...
from dynamic_network_architectures.building_blocks.residual import StackedResidualBlocks, CondBasicBlockD, BasicBlockD
from dynamic_network_architectures.building_blocks.helper import maybe_convert_scalar_to_list, get_matching_pool_op
from dynamic_network_architectures.building_blocks.simple_conv_blocks import StackedConvBlocks
from dynamic_network_architectures.building_blocks.simple_conv_blocks import CondConvDropoutNormReLU


class ConditionalResidualEncoder(nn.Module):
    def __init__(
        self,
        input_channels: int,
        n_stages: int,
        ...
        for s in range(n_stages):
            ...
            pools.append(pool_op(strides[s])) if pool_op is not None else pools.append(None)
    ...
    def forward(self, x: torch.Tensor, tembed: torch.Tensor = None):
        if self.stem is not None:
            if tembed is not None:
                x = self.stem(x, tembed)
            else:
                x = self.stem(x)
        ret = []
        for p, s in zip(self.pools, self.stages):
            if p is not None:
                x = p(x)
            if tembed is not None:
                x = s(x, tembed)
...
class CondPlainConvEncoder(nn.Module):
    def __init__(self,
        input_channels: int,
        n_stages: int,
        features_per_stage: Union[int, List[int], Tuple[int, ...]],
        ...
        pool: str = 'conv',
        time_embedding_dim: int = 512,
        block: CondConvDropoutNormReLU = CondConvDropoutNormReLU
    ):
        ...
        pool_op = get_matching_pool_op(conv_op, pool_type=pool) if pool != 'conv' else None
        ...
        for s in range(n_stages):
            if pool == 'conv':
                ...
                pools.append(None)
            elif (isinstance(strides[s], int) and strides[s] != 1) or \
                    isinstance(strides[s], (tuple, list)) and any([i != 1 for i in strides[s]]):
                pools.append(pool_op(strides[s], stride=strides[s])) if pool_op is not None else pools.append(None)
...
if __name__ == '__main__':
    ...
    model = ConditionalResidualEncoder(
        3, 5, (2, 4, 6, 8, 10), nn.Conv2d, 3, ((1, 1), 2, (2, 2), (2, 2), (2, 2)), 2, False,
